...
function ColorInput({ value, onChange }: { value: CanvasColor; onChange: (c: CanvasColor) => void }) {
  return (
    <div className="flex items-center gap-1.5">
      <input
        type="color" value={value.hex} className="w-6 h-6 rounded border border-border cursor-pointer p-0"
      ...
      <div className="flex items-center gap-1">
        ...
        <Slider value={[value.opacity * 100]} min={0} max={100} step={1} className="w-16"
...
function TextBlockEditor({ block, update }: { block: import('@binh-tran/shared').TextBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <div className="text-[10px] font-semibold uppercase text-muted-foreground tracking-widest">Content</div>
      <div className="border border-border rounded overflow-hidden min-h-[80px]">
      ...
      <Row label="Size">
        <NumInput value={block.fontSize} min={8} max={72} onChange={(v) => p({ fontSize: v })} />
      </Row>
      <Row label="Weight">
      ...
      <Row label="Color">
        <ColorInput value={block.color} onChange={(c) => p({ color: c })} />
      </Row>
      <Row label="Line H.">
        <NumInput value={block.lineHeight} min={0.8} max={4} step={0.05} onChange={(v) => p({ lineHeight: v })} />
      </Row>
      <Row label="Spacing">
        <NumInput value={block.letterSpacing} min={-0.1} max={0.5} step={0.01} onChange={(v) => p({ letterSpacing: v })} />
      </Row>
      <Row label="Margin↓">
        <NumInput value={block.marginBottom} min={0} max={80} onChange={(v) => p({ marginBottom: v })} />
      </Row>
    </div>
  )
}
...
function DateBlockEditor({ block, update }: { block: import('@binh-tran/shared').DateBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <Row label="Start">
        <Input type="month" value={block.startDate} className="h-7 text-xs" onChange={(e) => p({ startDate: e.target.value })} />
      </Row>
      <Row label="End">
        <Input type="month" value={block.endDate ?? ''} className="h-7 text-xs" disabled={block.current} onChange={(e) => p({ endDate: e.target.value })} />
      </Row>
      <Row label="Current">
        <Switch checked={block.current} onCheckedChange={(v) => p({ current: v })} />
      </Row>
      <Row label="Format">
      ...
      <Row label="Size">
        <NumInput value={block.fontSize} min={8} max={24} onChange={(v) => p({ fontSize: v })} />
      </Row>
    </div>
  )
}
...
function TagBlockEditor({ block, update }: { block: import('@binh-tran/shared').TagBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <div className="text-[10px] font-semibold uppercase text-muted-foreground tracking-widest">Tags</div>
      <div className="flex flex-wrap gap-1.5 min-h-[32px] p-2 border border-border rounded">
        {block.items.map((t, i) => (
          <span key={i} className="flex items-center gap-1 bg-muted px-2 py-0.5 rounded text-xs">
            {t}
      ...
      <Separator />
      <div className="text-[10px] font-semibold uppercase text-muted-foreground tracking-widest">Style</div>
...
function ProgressBlockEditor({ block, update }: { block: import('@binh-tran/shared').ProgressBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <Row label="Label"><Input value={block.label} className="h-7 text-xs" onChange={(e) => p({ label: e.target.value })} /></Row>
      <Row label="Value">
        <div className="flex items-center gap-2">
          <Slider value={[block.value]} min={0} max={100} className="flex-1"
            onValueChange={([v]) => p({ value: v })} />
          <span className="text-xs w-8 text-right">{block.value}%</span>
        </div>
      </Row>
...
function ImageBlockEditor({ block, update }: { block: import('@binh-tran/shared').ImageBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <Row label="URL"><Input value={block.url} placeholder="https://…" className="h-7 text-xs" onChange={(e) => p({ url: e.target.value })} /></Row>
...
function LinkBlockEditor({ block, update }: { block: import('@binh-tran/shared').LinkBlock; update: (p: Partial<CanvasBlock>) => void }) {
  ...
  return (
    <div className="space-y-2.5">
      <Row label="Label"><Input value={block.label} placeholder="Website" className="h-7 text-xs" onChange={(e) => p({ label: e.target.value })} /></Row>
      <Row label="URL"><Input value={block.url} placeholder="https://…" className="h-7 text-xs" onChange={(e) => p({ url: e.target.value })} /></Row>
      <Row label="Color"><ColorInput value={block.color} onChange={(c) => p({ color: c })} /></Row>
